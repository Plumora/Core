...
The Plumora Core daemon can be configured to act as a trusted "border
router", implementing the plumora wire protocol and relay, making
...
The ZeroMQ feature in Plumora Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building plumorad:
...
For instance:
    ...
    $ plumorad -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/plumorad.tx.raw \
...
These options can also be provided in plumora.conf.
...
From the perspective of plumorad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into plumorad directly. Furthermore, no information is
...
using. Plumorad appends an up-counting sequence number to each
