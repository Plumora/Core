Plumora Core version 0.18.1 is now available from:

  <https://download.plumora.org/plumora-0.18.1/>
...
`/Applications/Plumora-Qt` (on Mac) or `plumorad`/`plumora-qt` (on
Linux).
...
Plumora Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Plumora Core on unsupported systems.
...
10.10. Additionally, Plumora Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:20969:20969` (this is an extra `:20969` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@plumora.org) the Plumora Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 PLMR),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in PLMR per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/plumora-conf.md)
  about the `plumora.conf` file describes how to use it to configure
  Plumora Core.

- A new document introduces Plumora Core's BIP174 [Partially-Signed
  Plumora Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Plumora-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Plumora Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Plumora Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Plumora Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart plumorad with the `-deprecatedrpc=generate`
...
- A new `plumora-wallet` tool is now distributed alongside Plumora
...
This section describes planned changes to Plumora Core that may affect
other Plumora software and services.

- Since version 0.16.0, Plumora Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Plumora Core 0.20 (expected about
  a year after 0.18), Plumora Core will default to native segwit
  ...
  to bech32 addresses, and if the Plumora Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Plumora Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Plumora Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Plumora Core
...
- A sub-project of Plumora Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Plumora Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Plumora Core's own implementation, although entropy
  gathered by Plumora Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Plumora Core a
...
- On macOS, Plumora Core now opts out of application CPU throttling
...
- #20969 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
